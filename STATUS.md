...
### v0.13.1 (2026-08-16) — 镜头节奏修复：慢悠悠问题
- **慢预设微动态约束**: 9 个慢速预设（O2/O3/M2/M3/M5/M6/E1/E2/E3）各新增「微动态约束」字段，强制画面中有可见小幅运动（形变≥10%/雾气上升/光泽变化等），禁止纯静态
- **新增 2 个快节奏预设**: PRESET-M7（急推爆点特写，fast push-in + burst to freeze）+ PRESET-M8（快速横切对比，quick pan + burst to freeze）
- **预设矩阵强制穿插**: 软糯/液态/冰爽类轮换序列中强制穿插 M7/M8，保证每轮换至少 1 个快预设
- **video_prompt 节奏约束**: doubao_video_prompt.md 新增「节奏约束」段，禁止纯静态展示/禁止静止描述词/强制至少 1 个运动动词
- **产品动态禁忌**: system_prompt.md 新增 4 条产品动态禁忌，强制至少 1-2 帧爆发性动作
- **输出目录复用**: 生成图片时优先复用已有图片目录（main_window.py）
